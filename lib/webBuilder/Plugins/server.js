...
module.exports = {
    init:function(Site,callback){
        ...
        Site.server.getPage = function(jsonObject){
            ...
            $html.find('head').append('<title>'+jsonObject.title+'</title>').append($('<style></style>').html(Site.style.styleSheets['main'] ? Site.style.styleSheets['main'] : Site.style.createStyleSheet({name:'main'})));
